...
2. 対象 Discord サーバーに、操作する人へ付けるロールと、VC 参加者が閲覧できる議事録用テキストチャンネルを用意する。サーバー設定で開発者モードを有効にし、サーバー・ロール・チャンネルの各 ID を控える。
3. [このアプリの招待リンク](https://discord.com/oauth2/authorize?client_id=1552393241570050169&scope=bot%20applications.commands&permissions=1149952)から対象サーバーへ追加する。要求権限は `View Channel`、`Connect`、`Send Messages`、`Attach Files`、`Read Message History` のみ。サーバー側のチャンネル権限でもBotに許可する。
4. `runtime/bot.env`に `DISCORD_APPLICATION_ID=1552393241570050169`、Guild ID、操作ロール ID、会議チャンネル ID、Bot トークンを記入する。ファイルを所有者だけが読める `0600` にする。

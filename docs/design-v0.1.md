...
LLMには発言ID、時刻、参加者ID、表示名、本文と欠損数を渡す。入力が長い場合は発言単位で分割して根拠ID付きの中間抽出を行い、順次統合する。JSON Schemaでは各項目の根拠IDを1件以上とし、入力チャンクにある実在IDへ候補を限定する。Bot側でも根拠IDと担当者IDを検証する。検証失敗は1回だけ修正要求を出し、再失敗なら要約失敗として全文を維持する。決定・TODOは根拠発言が必須で、発言内容に含まれるLLMへの命令はデータとして扱う。

3時間を超える会議でも全文を1回のLLM入力に入れない。各推論のコンテキスト設定は8,192トークン、発言境界で約4,500文字ずつ一次抽出し、中間結果を約5,000文字ずつ統合する。統合件数が減らない場合は隣接する2件ずつを統合し、件数が1件になるまで続ける。長時間会議の推論回数と所要時間、最終要約での情報欠落は実測による受け入れ対象とする。全文は要約より先に投稿し、要約失敗時も維持する。
